...
int main(int argc, char **argv)
{
    ...
    #pragma acc enter data copyin(x[:dim], y[:dim], z[:dim], \
                                  q[:dim], potential[:dim])
    ...
    #pragma acc exit data copyout(potential[:dim])
...
{
    int stream = rand() % 4;
    #pragma acc parallel loop async(stream) present(x, y, z, q, potential)
    for (int i = 0; i < dim; ++i) { 
        double temp = 0.;
        ...
        for (int j = 0; j < dim; ++j) {
            if (i != j) {
                ...
                temp += pot_fun(dx, dy, dz, q[j], alpha);
            }
        }
